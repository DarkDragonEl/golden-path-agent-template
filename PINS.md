...
| buildah | ships with OpenShift Pipelines | bundled with `pipelines-1.22` | GA, Red Hat | 2026-08-21 | Red Hat "Securing OpenShift Pipelines" 1.20 docs (`buildah-ns` Task) | ClusterTasks are deprecated as of Pipelines 1.10+ (Tekton Resolvers replace them, GA since 1.11) — reference the Tekton Hub `buildah` `Task` via the `hub`/`bundle` resolver at implementation time, not a `ClusterTask` name. No separate pin needed beyond the operator itself. |
