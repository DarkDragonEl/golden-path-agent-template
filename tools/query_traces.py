...
Service DNS (http://golden-path-agent-otel-collector.golden-path-agent-otel.svc.cluster.local:8888/traces.jsonl)
-- reachable from inside the cluster (e.g. `oc exec` into any pod) or via
`oc port-forward svc/golden-path-agent-otel-collector 8888:8888 -n golden-path-agent-otel`
...
_DEFAULT_URL = (
    "http://golden-path-agent-otel-collector.golden-path-agent-otel.svc.cluster.local:8888/traces.jsonl"
)
