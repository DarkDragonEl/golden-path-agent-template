"""mcp_server/auth.py's get_authenticated_caller: identity+
...
def test_valid_token_passes_and_returns_subject(monkeypatch):
    private_key, public_key = _rsa_keypair()
    monkeypatch.setattr(auth, "_get_jwks_client", lambda issuer_url: _FakeJWKSClient(public_key))
    token = _make_token(private_key, sub="${{ values.name }}", aud=auth.MCP_AUDIENCE, iss=ISSUER)
    request = _make_request({"authorization": f"Bearer {token}"})

    assert auth.get_authenticated_caller(request) == "${{ values.name }}"
