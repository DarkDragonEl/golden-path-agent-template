...
def test_req_30052_findable_by_hyphenated_in_progress_status(store):
    # A hyphenated status value ("in-progress", mirroring how a model
    # sometimes formats it) must also match REQ-30052's seeded
    # "in_progress" status.
...
def test_req_30052_findable_by_space_separated_in_progress_status(store):
    # The generalized separator normalization must also cover a
    # space-separated variant ("in progress"), the third formatting
    # variant found during re-baseline testing.
    result = store.search(record_type="request", status="in progress")
    assert any(r["record_id"] == "REQ-30052" for r in result["records"])


def test_req_30052_findable_by_mixed_case_status(store):
    # The normalization also lowercases, covering case variation on top
    # of separator variation.
    result = store.search(record_type="request", status="In-Progress")
    assert any(r["record_id"] == "REQ-30052" for r in result["records"])
...
def test_inc_10234_findable_by_plural_ci_pipelines_query(store):
    # A plural query ("CI pipelines", mirroring how a
