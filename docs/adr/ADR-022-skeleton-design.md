...
distinct name/owner, using Backstage-native `values`/nunjucks syntax as the
single source of truth, rendered by a small, offline-testable renderer plus
RHDH's own stock `fetch:template` action — no custom RHDH plugin code. The
skeleton must also decide which source-repo content is generic template
content versus specific to this repository's own history or methodology.
...
`repoOwner`/`repoName`, load-bearing only for the publish stretch goal)
and a three-pass, longest-match-first substitution ordering (full repo
slug, then repo-name-only, then base-name). This repository's own
...
target) were never part of that curated content — a rendered project has
no `srs/` corpus at render time to check against.
...
- Nothing regenerates the skeleton from source, so nothing guarded its
  parity with it; `tools/skeleton_drift.py` is a read-only signal for that.
