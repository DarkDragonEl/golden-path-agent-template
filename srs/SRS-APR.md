...
- **SRS-APR-F-04 — Execution release, not execution.** On approval, the service shall release execution exclusively by invoking the tool-contract path with the *unmodified* approved arguments; the service itself shall contain no target-system client logic. `PROPOSED — pending owner review:` release is synchronous invoke-and-record (simplest for demo) vs. emitting an execution-authorized event consumed by the agent runtime. Rationale: synchronous keeps one audit record; event style decouples runtimes.
...
- **SRS-APR-SEC-02 — Approver authorization.** `PROPOSED — pending owner review:` a decision (SRS-APR-IF-02) shall be accepted only from an identity holding the approver role for the proposal's scope; a decision from an identity not so authorized shall be refused and audit-logged as a refused attempt, identically to SRS-APR-F-02's duplicate-decision handling. Rationale: this closes a gap identified during derivation — see `srs/FINDINGS.md` FIND-001. The SyRS records that decisions carry approver identity (SysR-P-F-08) and StRS defines the approver as a distinct stakeholder role (STK-05), but no SysR requires the service to verify the deciding identity actually holds that role. Alternative considered: rely on the demo UI being reachable only by designated approvers, with no service-side check — rejected as inconsistent with the fail-closed/least-privilege posture applied everywhere else in this document (SRS-APR-SEC-01, SysR-P-SEC-03).
...
*Requirements marked PROPOSED are not signed. The owner's review at Checkpoint B0-a resolves them; resolution is recorded by removing the marker and, where the decision was non-obvious, noting the rationale inline.*

**PROPOSED items in this document (4):** SRS-APR-F-04 (release mechanism: synchronous vs. event), SRS-APR-F-07 (idempotent submission), SRS-APR-SEC-02 (approver authorization — closes FIND-001), SRS-APR-SEC-04 (audit-record integrity).
