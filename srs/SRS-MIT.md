...
| Derivation basis | SyRS-AGP-001 v0.1 (frozen) — SysR-P-IF-02, SysR-P-IF-03; SysR-P-SEC-03 for the write-path boundary |
| Classification | Organization-agnostic blueprint; no proprietary content; no product names in normative text |
| Depth | **Interface-only** (per `MISSION_PHASE_B0.md`) — no functional, data, quality, or performance requirements. Demo scaffolding; not gold-plated. |

### Revision History

| Version | Date | Author | Changes |
|---|---|---|---|
| 0.1 | 2026-08-13 | Delivery agent (Phase B0) | Initial interface-only derivation, formalizing the provisional ITSM tool contract already established in `eval/README.md` at Phase A Checkpoint 1 (approved as Phase B0 input). Submitted at Checkpoint B0-a. |

### Associated Documents

- **SyRS-AGP-001 v0.1 (frozen)** — derivation basis.
...
- **`eval/cases/domain/itsm_read.yaml`, `draft_request.yaml`, `tool_selection.yaml`, `unauthorized_write.yaml`, `prompt_injection.yaml`** — Phase A cases that already exercise these tool names and argument shapes end-to-end (agent-observable evidence; see `srs/SRS-APR.md`'s evidence-precision convention, which applies here identically).
...
Per the working standard: sections of ISO 29148 §9.5 with no content for a blueprint demo component are marked *Not applicable — tailored out* with a one-line reason, never silently omitted. This document is **interface-only** by mission directive: Functional (§1), Data (§3), Quality, Verification, and Traceability sections are tailored out or omitted per the rules below — this is a scope decision made in `MISSION_PHASE_B0.md`, not a gap this document is silently absorbing. Verification methods: I (inspection), A (analysis), D (demonstration), T (test).
...
This document specifies **interface requirements only**: MCP tool schemas, the REST introspection surface, and state-visibility guarantees. It does not specify: the mock's internal matching/search logic (Phase B implementation detail), its persistence design — which store, if any (Phase B implementation detail; only the *externally observable* persistence guarantee is specified, at MIT-IF-05), performance targets, or usability/quality criteria. This is demo scaffolding standing in for a real enterprise ITSM system; SysR-P-IF-02's REST/OpenAPI-to-MCP bridge clause becomes relevant only if this mock is later replaced by a real ITSM integration that does not natively support MCP — not built here.
...
*Not applicable — tailored out.* Interface-only depth per `MISSION_PHASE_B0.md`; the mock's request-handling behavior (how a query matches records, how state is seeded/reset internally) is a Phase B implementation concern, not an interface contract.
