...
Bootstraps the ${{ values.name }} blueprint onto a fresh OpenShift cluster
...
# keycloak-cr.yaml's own header comment names ${{ values.name }}-keycloak-db-secret
# and ${{ values.name }}-keycloak-admin as "manually provisioned out-of-band" --
...
if ! oc get secret ${{ values.name }}-keycloak-db-secret -n ${{ values.name }}-keycloak >/dev/null 2>&1; then
  log "creating ${{ values.name }}-keycloak-db-secret (first time on this cluster)"
  oc create secret generic ${{ values.name }}-keycloak-db-secret \
    -n ${{ values.name }}-keycloak \
...
if ! oc get secret ${{ values.name }}-keycloak-admin -n ${{ values.name }}-keycloak >/dev/null 2>&1; then
  log "creating ${{ values.name }}-keycloak-admin (first time on this cluster)"
  oc create secret generic ${{ values.name }}-keycloak-admin \
    -n ${{ values.name }}-keycloak \
...
KC_TIMEOUT=300
while true; do
  KC_READY=$(oc get keycloak ${{ values.name }} -n ${{ values.name }}-keycloak \
    -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || echo "")
  [ "$KC_READY" = "True" ] && { log "  Keycloak: Ready"; break; }
  if [ "$KC_WAITED" -ge "$KC_TIMEOUT" ]; then
    log "  Keycloak: still not Ready after ${KC_TIMEOUT}s -- inspect 'oc get keycloak ${{ values.name }} -n ${{ values.name }}-keycloak -o yaml'"
    exit 1
  fi
...
RI_TIMEOUT=180
while true; do
  RI_DONE=$(oc get keycloakrealmimport ${{ values.name }} -n ${{ values.name }}-keycloak \
  ...
  if [ "$RI_WAITED" -ge "$RI_TIMEOUT" ]; then
    log "  KeycloakRealmImport: still not Done after ${RI_TIMEOUT}s -- inspect 'oc get keycloakrealmimport ${{ values.name }} -n ${{ values.name }}-keycloak -o yaml'"
    exit 1
  fi
...
# ${{ values.name }}-secrets in demo-prod itself if it doesn't yet exist
...
# failed CreateContainerConfigError): ${{ values.name }}-ci-config, a plain
...
NEEDS_MANUAL=false
if ! oc get secret ${{ values.name }}-secrets -n ${{ values.name }}-demo-prod \
    -o jsonpath='{.data.MODEL_API_KEY}' 2>/dev/null | grep -q .; then
  NEEDS_MANUAL=true
  log "  missing: ${{ values.name }}-secrets (MODEL_API_KEY) in ${{ values.name }}-demo-prod -- docs/phase-c-runbook.md S2"
fi
if ! oc get configmap ${{ values.name }}-ci-config -n ${{ values.name }}-ci >/dev/null 2>&1; then
  NEEDS_MANUAL=true
  log "  missing: ${{ values.name }}-ci-config in ${{ values.name }}-ci -- docs/phase-c-runbook.md S2b"
fi
...
ROOT_EXISTS=false
oc get applications.argoproj.io ${{ values.name }}-root -n openshift-gitops >/dev/null 2>&1 && ROOT_EXISTS=true
