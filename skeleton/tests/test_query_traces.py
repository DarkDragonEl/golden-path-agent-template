"""tools/query_traces.py's own parsing/filtering logic, pure
...
def test_filters_by_session_id():
    lines = [
        _span("agent.invoke", "${{ values.name }}", {"session.id": "sess-1"}),
        _span("agent.invoke", "${{ values.name }}", {"session.id": "sess-2"}),
    ]
...
def test_filters_by_proposal_id():
    lines = [
        _span("approval.decide_proposal", "${{ values.name }}-approval", {"proposal.id": "prop-1"}),
        _span("approval.decide_proposal", "${{ values.name }}-approval", {"proposal.id": "prop-2"}),
    ]
...
def test_results_sorted_chronologically_across_multiple_lines():
    lines = [
        _span("approval.decide_proposal", "${{ values.name }}-approval", {"session.id": "sess-1"}, start_ns=5000),
        _span("agent.invoke", "${{ values.name }}", {"session.id": "sess-1"}, start_ns=1000),
    ]
    records = find_matching_records(lines, session_id="sess-1", proposal_id=None)
...
def test_no_match_returns_empty_list():
    lines = [_span("agent.invoke", "${{ values.name }}", {"session.id": "sess-1"})]
