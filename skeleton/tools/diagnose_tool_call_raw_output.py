...
Classifies a raw tool-call failure mode into exactly one of:
...
diagnoses the failure mode as it actually stands under the current,
frozen config, not a hypothetical. Calls the model's raw HTTP endpoint directly
...
# One representative query per currently-failing category, verbatim
# from eval/cases/domain/*.yaml.
CASES = [
...
def build_user_message(query: str) -> str:
    ...
    the real model call uses, not a simplified stand-in."""
...
def extract_candidate_json(content: str):
    ...
    then a fenced ```json block. Returns
