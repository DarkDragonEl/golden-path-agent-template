...
Every attribute/event set here is read-only
...
hard constraint, not a style preference -- changing that would make
telemetry itself an undeclared change to what's actually sent to the
model.
"""
...
def _prompt_version(filename: str) -> str:
    ...
    embedded back into the prompt text itself (that would make a
    telemetry-only change perturb the prompt content, forcing an eval
    re-baseline every time telemetry changed). Out-of-band by
    construction: this hash reaches only the
...
def record_invocation_span(state: dict, request_id: str | None = None, span=None) -> None:
    ...
    # The attribute-correlation mechanism for stitching a trace across the
    # async approval gap -- session.id already covers the pre-proposal
    # portion; proposal.id
    ...
    # EVERY model call this turn -- state["model_calls"] (the
    ...
    # eval/domain_scorer.py's own compensating-control check for full
    # route coverage -- the route-coverage gap this closes on the
    # telemetry side).
