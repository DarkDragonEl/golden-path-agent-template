...
here from the pre-existing scaffold's snippet/source_uri to the
authoritative passage_text/source, plus the two fields the scaffold was
missing (owner_role, effective_date).
...
authorization-negative case to verify against (srs/SRS-AGT.md's own note
at SRS-AGT-F-02), so building enforcement here
...
def _words(text: str) -> set[str]:
    ...
    # a false-positive-retrieval finding: `[a-z0-9]+`
...
def retrieve(
    ...
    MIN_OVERLAP gates out noise matches: retrieval attaching a document
    on a single generic shared word (e.g.
