...
is wired into CI yet — it becomes a promotion gate only when the harness
...
previously unguarded — no model-failure fallback path existed in the
graph. The model call is now wrapped in try/except: `agent/model_client.py`'s
`RoutedModelClient` retries once against the configured fallback route on
any primary failure; on total failure (both routes exhausted, or none
configured),
...
(SRS-EVH-F-04's `PROPOSED` mechanism is not yet built into `trace-check`
itself) — this removal was done by hand,
